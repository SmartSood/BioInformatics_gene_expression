...
import { Experiment, ExperimentParameters, ExperimentResults } from '../utils/scemma';
  ...
  const fetchExperiments = async () => {
    try {
      ...
      if (stored) {
        ...
        localStorage.setItem('experiments', JSON.stringify(MOCK_EXPERIMENTS));
...
export function useExperimentDetails(experimentId: string | null) {
  ...
  const [parameters, setParameters] = useState<ExperimentParameters | null>(null);
  ...
  const fetchExperimentDetails = async (id: string) => {
    ...
    try {
      ...
      setParameters(storedParams ? JSON.parse(storedParams) : (MOCK_PARAMETERS[id] || null));
      setResults(storedResults ? JSON.parse(storedResults) : (MOCK_RESULTS[id] || null));
    } catch (error) {
